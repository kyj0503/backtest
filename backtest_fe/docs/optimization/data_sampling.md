...
**스마트 데이터 샘플링(Smart Data Sampling)**은 백테스트의 전체 기간에 따라 차트에 표시할 데이터의 집계 단위를 동적으로 조절하는 전략입니다. 기간이 짧을 때는 상세한 데이터를 보여주고, 기간이 길어질수록 데이터를 더 큰 시간 단위(주별, 월별)로 집계하여 렌더링할 데이터 포인트의 수를 최적의 범위 내로 유지합니다.

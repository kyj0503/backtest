...
> **⚠️ SUPERSEDED / 참고용 아카이브 (2026-08 표기)**
>
> 이 문서는 2025-11 시점에 작성된 **미실현 계획**입니다. 아래 "완료 현황"에 적힌 Phase 1/2.1/4.1은 그 시점 기준이며,
> 특히 "Zustand + useReducer 상태 관리 패턴"이라는 서술은 **사실이 아니었습니다** — 이 저장소에 Zustand가 의존성으로
> 들어간 적은 없습니다 (`git log --all -S '"zustand"' -- backtest_fe/package.json`는 공집합). 현재 상태 관리
> 방식은 [`../architecture/state_management.md`](../architecture/state_management.md)를 참고하세요.
> Phase 2.2 이후(데이터 변환 유틸, 공통 UI 컴포넌트, Phase 3~6)는 계획 단계에서 더 진행되지 않았습니다.
> 아래 본문은 당시 계획을 그대로 보존한 히스토리이며, 현재 아키텍처 문서로 사용하지 마세요.

**작성일**: 2025-11-16
**최종 수정일**: 2025-11-18
**상태**: Phase 1, 2.1, 4.1 완료 (컴포넌트 분할, 로직 분리, 타입 안전성 강화) — 그 이후 진행되지 않음, 위 SUPERSEDED 안내 참고
...
- useReducer 기반 지역 상태 관리 패턴 (전역 상태 라이브러리는 미도입 — 위 SUPERSEDED 안내 참고)

pipeline {
    ...
    stages {
        ...
        stage('Dependency Audit') {
            steps {
                script {
                    // P3-14: 각 Dockerfile의 audit 스테이지를 빌드해 lock 파일
                    // 기준으로 취약점을 조회한다. Quality Gate와 같은 방식
                    // (빌드 컨텍스트 스트리밍)을 쓴다 — 이 Jenkins는 컨테이너
                    // 안에서 호스트 데몬에 붙어 sibling 컨테이너를 띄우므로
                    // docker run -v의 소스 경로가 호스트 기준으로 해석되어
                    // 워크스페이스 파일을 찾지 못한다 (빌드 #20 실패 원인).
                    //
                    ...
                    // 예외 목록을 비우면 실제로 종료코드 1이 나오는 것을 확인했다.
                    parallel(
                        'Frontend (npm audit)': {

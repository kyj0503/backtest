...
> **문서 구성**
> - **Part 1 — 활성 백로그 (2026-08-08)**: 지금 해야 할 일. 여기부터 읽으면 된다.
> - **Part 2 — 완료 기록 (2026-08-02 라운드)**: 이전 감사에서 처리한 항목의 이력.
>   대부분 `[x]`이며, 왜 그렇게 고쳤는지의 근거가 남아 있어 보존한다.

---

# Part 1 — 활성 백로그 (2026-08-08 통합)
...
- [ ] **A-03 [be]** buy&hold 경로의 데이터 로드 실패가 무경고 원금 증발로 나타남 〔C-02 / 기존 P2-06〕
  ...
  > 이 항목은 Part 2의 P2-06과 같은 건이다. 그쪽 체크박스가 `[x]`인데 본문은
  > "⏸ 미착수"였다 — 2026-08-08 통합 시 `[ ]`로 정정하고 여기로 이관했다.
...
- [ ] **A-09 [be]** 같은 응답 필드가 실행 경로마다 다른 의미를 가짐 〔C-03〕
  ...
  정의가 바뀐다.** 특히 `2.0 if gross_profit > 0 else 1.0` 폴백은 Part 2의 P3-21에서
...
두 세션 모두 동일하게 지목했고, Part 2의 "저장소 밖 필수 후속 조치"와도 이어진다.
...
Part 2의 "변경 시 공통 완료 기준"을 그대로 승계한다 — 수정 전 실패를 재현하는 테스트가
...
>
> **아래는 이력이다.** 대부분 완료됐고, 미완 항목은 Part 1으로 이관했다(P2-06 → A-03).
...
- [ ] **P2-06 [be]** ⏸ 미착수 → **Part 1의 A-03으로 이관** (2026-08-08 통합 시 체크박스 정정: 본문이 "미착수"인데 `[x]`로 표기돼 있었다) — buy&hold 데이터 로드 실패 종목 무경고 드랍 — 자본은 분모 잔류로 수익률 과소보고, 전략 경로와 달리 warnings 필드 없음(`portfolio_data_loader.py:52-59`). 조치: warnings 통일 또는 실패 처리.

...
**역할**:
- FastAPI 엔드포인트의 요청 본문을 검증하는 Pydantic 모델
- 타입 안전성 및 자동 문서화 제공
- 유효성 검사 규칙 정의

**주요 모델**:
1. BacktestRequest: 단일 종목 백테스트 요청
   - ticker, start_date, end_date: 필수
   - strategy, strategy_params: 전략 설정
   - initial_cash, commission: 백테스트 설정

2. PortfolioBacktestRequest: 포트폴리오 백테스트 요청 (schemas.py 참조)
...
**연관 컴포넌트**:
- Backend: app/api/v1/endpoints/backtest.py (모델 사용)
- Backend: app/models/responses.py (응답 모델)
- Backend: app/schemas/schemas.py (추가 스키마)
- Frontend: src/features/backtest/model/api-types.ts (TypeScript 타입)

...
- OKLCH 색상 형식 지원 브라우저 (Chrome 111+, Firefox 113+, Safari 15.4+)
...
1. 루트 엘리먼트에 `.dark` 클래스가 올바르게 토글되는지 확인
2. 테마 JSON 파일에 `dark` 색상 변수가 정의되어 있는지 확인
3. CSS에서 `var(--변수명)` 형식으로 색상을 사용하고 있는지 확인

## 최근 수정사항 (2025-09-13)

### 문제점 해결
- **다크모드 적용 문제**: CSS 변수가 `hsl()` 래퍼 없이 직접 사용되도록 수정
- **테마 변경 미적용**: 하드코딩된 색상 클래스를 CSS 변수 기반 클래스로 전면 교체
- **OKLCH 형식 지원**: 테마 JSON의 OKLCH 색상 형식을 index.css에서 직접 지원

### 수정된 컴포넌트
- `StockAutocomplete`: 배경색, 테두리, 호버 효과를 테마 변수로 변경
- `SearchableSelect`: 드롭다운 스타일을 테마 기반으로 변경
- `FormField`: 에러 상태 색상을 `destructive` 변수로 변경
- `ChartLoading`: 배경색을 `muted` 변수로 변경
- `ToggleSwitch`: 활성/비활성 색상을 테마 변수로 변경

### CSS 구조 개선
- CSS 변수를 `hsl()` 래퍼 없이 직접 사용하도록 수정
- OKLCH 색상 형식을 기본값으로 설정
- Tailwind CSS 클래스명을 shadcn/ui 표준에 맞춤
